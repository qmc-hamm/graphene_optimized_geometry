...
if __name__=="__main__":
    ...
    theta1 = [4.4, 3.89, 2.88, 2.0, 1.47, 1.41, 1.35, 1.29, 1.25, 1.2, 1.16, 1.08, 1.05, 0.99, 0.93, 0.84]#Input: Twist angle (Float)
    for i in range(len(theta1)):
        theta = theta1[i]
        theta2 = str(theta1[i])
        st = theta2.split('.')
        thetaname = st[0]+'-'+st[1]
        method = "MD-REBO-KC-qmcfitted" #Input: method (String)
        ID = str(uuid.uuid4()) 
        date = 'Nov 3, 2021' #Input:date (String)
