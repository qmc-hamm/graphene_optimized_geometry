...
def get_data(twist_angle, potential, atom_type=1):
    ...
    # ax = scatterplotter(ax, d_rotated.x, d_rotated.y, d_rotated.energy, label, strip_width=strip_width, vmin=-7.427, vmax=-7.412)
    # plt.savefig(f'{twist_angle}_{potential}_energy.pdf')
    # plt.close()
    return d_rotated

def scatterplotter(ax, x, y, z, title, strip_width=None, vmin=None, vmax=None):
    p = ax.scatter(x, y, c=z, cmap='jet', s=2, alpha=0.8, lw=0)
...
def plot_energy_side_by_side(twist_angle, pot1, pot2, label1, label2, atom_type=1, strip_width=None):
    d1 = get_data(twist_angle, pot1, strip_width=strip_width)
    d2 = get_data(twist_angle, pot2, strip_width=strip_width)
    fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(7, 5))

    cax = fig.add_axes([
        ax1.get_position().x1 + 0.01 + 0.1,
        ax1.get_position().y0 + 0.05,
        0.02,
        ax1.get_position().height
        ])
    p, ax1 = scatterplotter(ax1, d1.x, d1.y, d1.energy, label1, strip_width=strip_width, vmin=-7.427, vmax=-7.412)
    p, ax2 = scatterplotter(ax2, d2.x, d2.y, d2.energy, label2, strip_width=strip_width, vmin=-7.427, vmax=-7.412)
    cbar = fig.colorbar(p, cax=cax)
    # cbar = plt.colorbar(p, cax=cax)

    # cbar.set_label('Energy (eV/atom)', rotation=90, fontsize=9, fontweight='bold', labelpad=8)
    # cbar.ax.tick_params(labelsize=9)
    # p.set_clim(vmin=vmin, vmax=vmax)
    plt.savefig(f'{twist_angle}_side_by_side.pdf')
...
if __name__ == '__main__':
    ...
    for twist_angle in ['0-99']:
        ...
        plot_energy_side_by_side(twist_angle, 'ouyang', 'refit', '(a) Ouyang', '(b) QMC', atom_type=1, strip_width=None)
